  ...
  public play = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (gameParticipation.timesPlayed >= game.playLimitPerUser) {
        return res.status(400).json({ message: 'You have reached the limit for this game today.' });
      }
      ...
      if (user.prizesWonToday && user.prizesWonToday.includes(gameId)) {
        ...
        return res.json({ message: 'You have already won a prize for this game today.' });
      }
      ...
      if (!prize || prize.rules.totalSupply <= 0 || prize.rules.dailyLimit <= 0) {
        await this.userService.updateUser(user._id, user);
        return res.json({ message: 'No prize to be given at this moment.' });
      }
      ...
      res.json({ prize, message: 'Congratulations! You have won a prize.' });
    } catch (error) {
      console.log(error);
    }
  };
